...
void check_t_paths(t_data *data)
{
	...
	if (!data->paths.pn || !data->paths.ps || !data->paths.pw || !data->paths.pe)
	...
	if (data->paths.pn[len - 4] != '.' || data->paths.pn[len - 3] != 'x' || data->paths.pn[len - 2] != 'p' || data->paths.pn[len - 1] != 'm')
		exit(ft_exit(data, "Error: Missing .xpm extension"));
	len = ft_strlen(data->paths.ps);
	if (data->paths.ps[len - 4] != '.' || data->paths.ps[len - 3] != 'x' || data->paths.ps[len - 2] != 'p' || data->paths.ps[len - 1] != 'm')
		exit(ft_exit(data, "Error: Missing .xpm extension"));
	len = ft_strlen(data->paths.pw);
	if (data->paths.pw[len - 4] != '.' || data->paths.pw[len - 3] != 'x' || data->paths.pw[len - 2] != 'p' || data->paths.pw[len - 1] != 'm')
		exit(ft_exit(data, "Error: Missing .xpm extension"));
	len = ft_strlen(data->paths.pe);
	if (data->paths.pe[len - 4] != '.' || data->paths.pe[len - 3] != 'x' || data->paths.pe[len - 2] != 'p' || data->paths.pe[len - 1] != 'm')
		exit(ft_exit(data, "Error: Missing .xpm extension"));
}